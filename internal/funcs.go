...
// colnamesupdatequery creates a list of the column names in fields as a query and
// joined by sep, excluding any Field with Name contained in ignoreNames.
//
...
// $2, ...").
//
// IF forUpdate is true then fields that are marked as `IsIgnoredOnUpdate` will be omitted
func (a *ArgType) colnamesquerymulti(fields []*Field, sep string, startCount int, ignoreNames []*Field, forUpdate bool) string {
	ignore := map[string]bool{}
	for _, f := range ignoreNames {
		ignore[f.Name] = true
	}
